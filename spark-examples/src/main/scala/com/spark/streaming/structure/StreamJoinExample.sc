...
case class sensor(carId:String,devicename:String,id:Int,ip:String,description:String,temp:Long,co2level:Long,lat:Double,longi:Double,eventTime:Timestamp)
  ...
  def main(args: Array[String]): Unit =
  {
    ...
    while(true)
    {
      ...
      csv = spark.read.option("header", "true")
        ...
        .csv("/Users/nd2629/Desktop/workspace/KafkaSparkStreams/KafkaApps/resources/mapfile.csv")
        .as[location]
    }
    ...
    def startQuery(spark: SparkSession,topic: String,ds:Dataset[location]):StreamingQuery=
    {
      ...
      val df: DataFrame = spark.readStream
        ...
        .option("subscribe", sourceTopic).option("startingOffsets", "latest")
      ...
      ////result.writeStream.outputMode("complete").format("console").option("truncate","false").start()          result.writeStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092")
      val result = ds.join(csv,Seq("id"))
        .option("topic", endTopic)
        .option("checkpointLocation", "/tmp/sparkcheckpoint1/")
        .queryName("kafka spark streaming kafka1")
        .outputMode("update")
        .start()
